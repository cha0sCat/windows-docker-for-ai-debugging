...
- `scripts/provision.sh`: Linux provisioning (Docker + nvm + Node 20 + Codex), then starts the selected VM and waits for SSH.
- `scripts/provision-win.ps1`: Windows provisioning via WSL2 (Node + Codex on Windows; runs `scripts/provision.sh` inside WSL to start the VM).
